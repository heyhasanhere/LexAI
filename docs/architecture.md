...
| LLM | Mistral-Small-3.1-24B-Instruct via vLLM | Fits in 24GB VRAM (2x RTX 3060) at bfloat16 with tensor-parallel-size 2; strong structured JSON output and instruction-following; vLLM exposes an OpenAI-compatible API so no custom client code needed |
| Embedding | BAAI/bge-large-en-v1.5 via sentence-transformers | Best quality local English embedding model; 335M params, runs on a single RTX 3060; no API cost |
